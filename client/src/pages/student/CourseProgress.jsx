...
const CourseProgress = () => {
  ...
  const { courseTitle } = courseDetails;
     ...
     const initialLecture = currentLecture || (courseDetails?.lectures?.length ? courseDetails.lectures[0] : null);
        ...
        <div className="flex-1 md:w-3/5 h-fit rounded-lg shadow-lg p-4">
          <div>
            {!initialLecture?.videoUrl ? (
              <p className="text-red-500">No video URL found</p>
            ) : (
              <video
                key={`${courseId}-${currentLecture?._id || initialLecture._id}`}
                src={initialLecture.videoUrl.replace("http://", "https://")}
                controls
